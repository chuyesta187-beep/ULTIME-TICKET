...
client.on('ready', async () => {
    // Al no especificar Guild ID se registra globalmente (puede tardar unos minutos en aparecer en todos lados)
    // Si quieres pruebas instantáneas en un servidor, usa: client.guilds.cache.get('ID_SERVER')?.commands.create(...)
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'send-panel-ticket') {
        const embedPanel = new EmbedBuilder()
            ...
            .setColor('#2b2d31') // Color gris oscuro estético
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'select_ultime_categoria') {
        ...
        // Configuramos campos según la categoría elegida
        ...
        } else { // Configuración y Otras Consultas usan este diseño general
            modal.addComponents(
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_ultime_')) {
        ...
        const randomID = Math.floor(1000 + Math.random() * 9000); // ID única de 4 dígitos
        ...
        // Recolectar las respuestas dinámicamente
        ...
        } catch (e) { /* Evitar caídas si cambian estructuras */ }

        // Creación del Canal Privado
        const canalTicket = await interaction.guild.channels.create({
            name: `ticket-${interaction.user.username}`,
            type: ChannelType.GuildText,
            parent: CONFIG.CATEGORY_TICKETS,
            permissionOverwrites: [
                { id: interaction.guild.id, deny: [PermissionFlagsBits.ViewChannel] }, // Ocultar a Everyone
                { id: interaction.user.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }, // Usuario
                { id: CONFIG.STAFF_ROLE, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] } // Staff
            ]
        });

        // Embed del Ticket creado
    ...
    // 👤 ACCIÓN: RECLAMAR TICKET
    if (interaction.isButton() && interaction.customId.startsWith('ultime_reclamar_')) {
        // Verificar si el usuario que pulsa tiene el rol de Staff configurado
        if (!interaction.member.roles.cache.has(CONFIG.STAFF_ROLE)) {
            return interaction.reply({ content: '❌ No formas parte del equipo de Staff para reclamar este ticket.', ephemeral: true });
        }

        const embedOriginal = EmbedBuilder.from(interaction.message.embeds[0]);
        
        // Actualizar campo de "Reclamado por:" (Index 4 en los fields agregados)
        embedOriginal.spliceFields(4, 1, { name: '👤 Reclamado por:', value: `${interaction.user}`, inline: true });

        // Deshabilitamos únicamente el botón de reclamar modificando la fila de componentes
    ...
    // 🔒 ACCIÓN: CERRAR TICKET + TRANSCRIPCIÓN AUTOMÁTICA
    if (interaction.isButton() && interaction.customId.startsWith('ultime_cerrar_')) {
        ...
        // Recolectar todos los mensajes del canal (Hasta un límite de 100 mensajes)
        ...
        // 📜 LOGS: Ticket Cerrado & Envío de archivo de Transcripción
        const canalLogs = interaction.guild.channels.cache.get(CONFIG.LOGS_CHANNEL);
        if (canalLogs) {
        ...
        // Eliminar el canal del ticket tras 3 segundos
        setTimeout(async () => {
